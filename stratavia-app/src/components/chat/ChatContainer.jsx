...
export default function ChatContainer() {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#fcf8fa] font-sans text-[#1b1b1d]">
      <SideNav />
    ...
    <aside className="hidden h-screen w-64 shrink-0 flex-col border-r border-[#c6c6cd] bg-[#f0edef] py-2 md:flex">
      ...
      <nav className="flex-1 space-y-1">
        {navItems.map((item) => (
          <button
            key={item.label}
            className={`flex w-full items-center border-r-4 px-6 py-3 text-left transition ${
              item.active
      ...
      <div className="mt-auto px-6">
        <div className="mb-6 rounded-lg bg-[#131b2e] p-4 text-center shadow-[0_4px_6px_-1px_rgba(15,23,42,0.05),0_2px_4px_-2px_rgba(15,23,42,0.05)]">
          ...
          <button className="w-full rounded bg-[#006c49] py-2 font-mono text-xs font-semibold leading-4 tracking-[0.05em] text-white transition hover:bg-[#005236]">
            Upgrade to Pro
          </button>
        </div>
  ...
  return (
    <header className="z-10 flex h-20 shrink-0 items-center justify-between border-b border-[#c6c6cd] bg-[#fcf8fa]/85 px-4 backdrop-blur-md md:px-10">
      ...
      <div className="flex items-center gap-4 md:gap-6">
        ...
        <button className="text-[#45464d] transition hover:text-[#006c49]">
...
function Greeting() {
  return (
    <div className="mb-2 flex flex-col items-center text-center">
      <div className="mb-4 flex h-16 w-16 items-center justify-center rounded-lg bg-[#006c49] text-white shadow-[0_4px_6px_-1px_rgba(15,23,42,0.05),0_2px_4px_-2px_rgba(15,23,42,0.05)]">
...
function ChatMessage({ message }) {
  ...
  return (
    <div className="flex max-w-3xl gap-4">
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded bg-[#006c49] text-white">
      ...
      <div className="min-w-0 flex-1 space-y-4">
        ...
        {message.followUp && (
          <p className="text-base leading-7 text-[#1b1b1d]">{message.followUp}</p>
        )}
      </div>
    </div>
  );
...
function ComparisonTable() {
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-[#c6c6cd] bg-white shadow-[0_4px_6px_-1px_rgba(15,23,42,0.05),0_2px_4px_-2px_rgba(15,23,42,0.05)]">
      <table className="w-full text-left text-sm leading-5">
        ...
        <tbody className="divide-y divide-[#c6c6cd]">
          {rows.map(([label, portugal, paraguay]) => (
            <tr key={label} className="odd:bg-white even:bg-[#fcf8fa]">
          ...
          <div className="relative rounded-2xl border-2 border-[#c6c6cd] bg-white p-4 shadow-sm transition focus-within:border-[#006c49] focus-within:ring-4 focus-within:ring-[#006c49]/5">
            ...
            <div className="mt-2 flex items-center justify-between border-t border-[#c6c6cd]/30 pt-2">
              <div className="flex gap-2">
                {["attach_file", "image", "description"].map((icon) => (
                  <button
                    key={icon}
...
function UserAvatar() {
  return (
    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#131b2e] text-sm font-semibold text-white">
      AR
    </div>
  );
}
  ...
  const icons = {
    ...
    settings: (
      <>
        ...
        <path d="M19.4 15a1.7 1.7 0 0 0 .34 1.88l.06.06-2 3.46-.08-.03a1.7 1.7 0 0 0-1.94.35l-.13.12a1.7 1.7 0 0 0-.48 1.54V22h-4v-.1a1.7 1.7 0 0 0-.5-1.53l-.12-.12a1.7 1.7 0 0 0-1.94-.35l-.08.03-2-3.46.06-.06A1.7 1.7 0 0 0 4.6 15v-.17a1.7 1.7 0 0 0-1.1-1.6L3.4 13v-4l.1-.03a1.7 1.7 0 0 0 1.1-1.6V7.2a1.7 1.7 0 0 0-.34-1.88l-.06-.06 2-3.46.08.03a1.7 1.7 0 0 0 1.94-.35l.13-.12A1.7 1.7 0 0 0 8.83 0H13v.1a1.7 1.7 0 0 0 .5 1.53l.12.12a1.7 1.7 0 0 0 1.94.35l.08-.03 2 3.46-.06.06a1.7 1.7 0 0 0-.34 1.88v.17a1.7 1.7 0 0 0 1.1 1.6l.1.03v4l-.1.03a1.7 1.7 0 0 0-1.1 1.6z" />
      </>
    ),
    attach_file: (
